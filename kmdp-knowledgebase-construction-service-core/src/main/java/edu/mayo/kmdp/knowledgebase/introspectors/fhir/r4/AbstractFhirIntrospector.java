...
import edu.mayo.kmdp.language.parsers.fhir.r4.FHIR4Deserializer;
...
public abstract class AbstractFhirIntrospector<T extends Resource> extends
  ...
  static final _applyLift parser = new FHIR4Deserializer();
  ...
  protected Answer<KnowledgeCarrier> doIntrospect(
    ...
    Answer<T> vsOpt = parser.applyLift(knowledgeCarrier, Abstract_Knowledge_Expression.getTag(),
        codedRep(FHIR_R4), null).flatOpt(kc -> kc.as(getTypeClass()));
    if (vsOpt.isFailure()) {
