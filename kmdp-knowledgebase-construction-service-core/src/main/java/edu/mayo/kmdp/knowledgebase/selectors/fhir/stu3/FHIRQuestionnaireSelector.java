...
public class FHIRQuestionnaireSelector
    extends AbstractKnowledgeBaseOperator
    implements _applyNamedSelect {

  public static final UUID id = UUID.fromString("e036ea56-37ec-4652-a92b-cde0047edfbe");
  public static final String version = "1.0.0";

  public FHIRQuestionnaireSelector() {
    super(SemanticIdentifier.newId(id, version));
  }

  public FHIRQuestionnaireSelector(KnowledgeBaseApiInternal kbManager) {
    this();
  ...
  public Answer<KnowledgeCarrier> applyNamedSelect(UUID operatorId, KnowledgeCarrier definition,
    ...
    return kbManager.getKnowledgeBaseManifestation(kbaseId,versionTag)
        .flatOpt(kc -> kc.as(Questionnaire.class))
        .map(q -> visit(q))
        .map(vs -> filter(vs,definition));
  }

  private KnowledgeCarrier filter(ValueSet vs, KnowledgeCarrier definition) {
    ValueSet def = definition.as(ValueSet.class).orElseThrow(UnsupportedOperationException::new);
  ...
  private ValueSet visit(Questionnaire quest) {
    ValueSet vs = new ValueSet();
    quest.getItem().stream()
        .flatMap(this::flattenQuestionnaire)
        .flatMap(qic -> qic.getCode().stream())
  ...
  private Stream<QuestionnaireItemComponent> flattenQuestionnaire(
      QuestionnaireItemComponent root) {
    return Stream.concat(Stream.of(root),
        root.getItem().stream().flatMap(this::flattenQuestionnaire));
  }
