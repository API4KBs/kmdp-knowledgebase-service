...
public final class FHIRPlanDefinitionUtils {
  ...
  public static void setKnowledgeIdentifiers(
      PlanDefinition pd,
      ResourceIdentifier assetId,
      ResourceIdentifier rootId,
      ResourceIdentifier artifactId) {

    var id1 = toFHIRIdentifier(assetId,
        newTerm(URI.create("https://www.omg.org/spec/API4KP/"), "KnowledgeAsset"));
    var id2 = toFHIRIdentifier(artifactId,
            newTerm(URI.create("https://www.omg.org/spec/API4KP/"), "KnowledgeArtifact"));
    var id3 = toFHIRIdentifier(rootId,
        newTerm(URI.create("https://www.omg.org/spec/API4KP/"), "KnowledgeAsset"),
        false);
    pd.setIdentifier(id3 != null ? List.of(id1, id2, id3) : List.of(id1, id2));
    if (assetId != null) {
      pd.setUrl(assetId.getVersionId().toString());
    }
  }
  ...
  public static void setKnowledgeIdentifiers(
      ...
      ResourceIdentifier assetId,
      ResourceIdentifier artifactId) {
    setKnowledgeIdentifiers(pd, assetId, null, artifactId);
  }

  public static Identifier toFHIRIdentifier(ResourceIdentifier id, Term type) {
    return toFHIRIdentifier(id, type, true);
  }

  public static Identifier toFHIRIdentifier(ResourceIdentifier id, Term type, boolean primary) {
    if (id == null) {
      return null;
    }
    return new Identifier()
        .setType(toCodeableConcept(type))
        .setUse(primary ? IdentifierUse.OFFICIAL : IdentifierUse.SECONDARY)
